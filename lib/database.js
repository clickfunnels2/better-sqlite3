...
function Database(filenameGiven, options) {
	...
	if (Buffer.isBuffer(filenameGiven)) {
		...
		if(options?.name) {
			dbName = options?.name
			queries.push(['mode', 'memory']);
		}
		
		if(options?.shared) {
			queries.push(['cache', 'shared']);
		}

		if(queries.length > 0) {
		  filenameGiven = `file:${dbName}?${queries.map(([query, value]) => `${query}=${value}`).join('&')}`;
		} else {
			filenameGiven = dbName;
		}
	...
	const memdb = filename.includes("vfs=memdb")
	const readonly = util.getBooleanOption(options, 'readonly');
	const fileMustExist = util.getBooleanOption(options, 'fileMustExist');
	const timeout = 'timeout' in options ? options.timeout : 5000;
	const verbose = 'verbose' in options ? options.verbose : null;
	const shared = 'shared' in options ? options.shared : filenameGiven.includes("cache=shared");
	...
	// verbose && verbose("Oppening connection to filename: ", filename, {shared})
	...
	if (!anonymous && !memdb && !fs.existsSync(path.dirname(filename))) {
		throw new TypeError('Cannot open database because the directory does not exist');
	}

	Object.defineProperties(this, {
		[util.cppdb]: { value: new addon.Database(filename, filenameGiven, anonymous, readonly, fileMustExist, timeout, verbose || null, buffer || null, shared) },
		...wrappers.getters,
	});
}
